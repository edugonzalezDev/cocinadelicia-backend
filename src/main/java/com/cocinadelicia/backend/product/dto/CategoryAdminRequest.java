...
public record CategoryAdminRequest(
    @NotBlank @Size(max = 191) @Schema(description = "Nombre visible de la categoría", example = "Empanadas")
        String name,
    @NotBlank @Size(max = 191) @Schema(description = "Slug único (URL friendly)", example = "empanadas")
        String slug,
    @Size(max = 500) @Schema(
