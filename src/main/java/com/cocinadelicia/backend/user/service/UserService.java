...
import com.cocinadelicia.backend.auth.cognito.CognitoUserInfoClient; // 👈 NUEVO
...
public class UserService {
    ...
    @Transactional
    public UserResponseDTO registerOrUpdateFromToken(UserRegistrationDTO body, JwtAuthenticationToken auth) {
        ...
        final List<String> groups = Optional.ofNullable((List<String>) jwt.getClaim("cognito:groups"))
                .orElseGet(List::of);
        ...
        String givenName = firstNonBlank(asString(userInfo.get("given_name")), body != null ? body.getFirstName() : null);
        String familyName = firstNonBlank(asString(userInfo.get("family_name")), body != null ? body.getLastName() : null);
        String phone = firstNonBlank(asString(userInfo.get("phone_number")), body != null ? body.getPhone() : null);

        if (rawEmail == null || rawEmail.isBlank()) {
            // devolvemos 400 con mensaje claro (agrega handler si quieres)
            throw new MissingEmailException("Cognito userInfo did not return email. Ensure scopes 'email profile' "
    ...
    private void addMissingRoles(AppUser user, List<String> groups) {
        ...
        Set<RoleName> requested = groups.stream()
                .filter(Objects::nonNull)
                .map(String::trim)
                .map(String::toUpperCase)
                .map(s -> {
                    try { return RoleName.valueOf(s); }
        ...
        Map<RoleName, Role> roleByName = existing.stream()
                .collect(Collectors.toMap(Role::getName, r -> r));

        for (RoleName rn : requested) {
            Role r = roleByName.get(rn);
            if (r == null) continue;
            boolean hasIt = user.getRoles().stream().anyMatch(ur -> ur.getRole().getId().equals(r.getId()));
            if (!hasIt) {
    ...
    public static class MissingEmailException extends RuntimeException {
        public MissingEmailException(String msg) { super(msg); }
    }
}
